...
async function create(userId, oldToken, expectedPasswordHash) {
  return withTransaction(async client => {
    ...
    const { rows } = await client.query("SELECT id FROM users WHERE id = $1 AND status = 'active' AND deleted_at IS NULL AND email_verified_at IS NOT NULL AND password_hash = $2 FOR UPDATE", [userId, expectedPasswordHash]);
    if (!rows[0]) return null;
    if (TOKEN.test(oldToken || '')) await client.query('DELETE FROM auth_sessions WHERE token_hash = $1', [digest(oldToken)]);
    await client.query('DELETE FROM auth_sessions WHERE user_id = $1 AND expires_at <= NOW()', [userId]);
    const token = crypto.randomBytes(32).toString('hex');
    const csrfToken = crypto.randomBytes(32).toString('hex');
    await client.query("INSERT INTO auth_sessions (token_hash, user_id, csrf_token, expires_at) VALUES ($1, $2, $3, NOW() + INTERVAL '24 hours')", [digest(token), userId, csrfToken]);
...
async function read(token) {
  ...
  const { rows } = await query(`SELECT s.csrf_token, u.id, u.email FROM auth_sessions s JOIN users u ON u.id = s.user_id
    WHERE s.token_hash = $1 AND s.expires_at > NOW() AND u.status = 'active' AND u.deleted_at IS NULL AND u.email_verified_at IS NOT NULL`, [digest(token)]);
  if (!rows[0]) return null;
