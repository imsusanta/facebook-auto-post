...
router.post('/post', upload.single('image'), validatePost, async (req, res, next) => {
  ...
  try {
    ...
    const target = req.body.facebookPageId ? await storage.getPageById(req.body.facebookPageId) : await storage.getActivePage();
    if (!target) return res.status(404).json({error:'Connected page not found'});
    const result = await context.run(context.current().workspaceId, () => facebook.publishPost({
      message, imagePath, imageUrl, isDemo, source: 'manual'
    }), {...context.current(), targetPageId: target.id});

    broadcastSSE('history_updated', (await storage.getHistory()));
    res.json(result);
  } catch (err) {
    broadcastSSE('history_updated', (await storage.getHistory()));
    next(err);
...
router.post('/pages', async (req, res, next) => {
  ...
  try {
    ...
    if (info.pageId !== pageId.trim()) return res.status(400).json({error: 'Token does not belong to the requested page'});
    ...
    broadcastSSE('page_switched', { activePage: (await storage.getActivePage()), pages: (await storage.getConnectedPages()) });
...
router.post('/pages/switch', async (req, res) => {
  ...
  broadcastSSE('page_switched', { activePage: switched, pages: (await storage.getConnectedPages()) });
...
router.put('/pages/:id', async (req, res, next) => {
  ...
  try {
    ...
    if (accessToken) {
      ...
      if (info.pageId !== req.params.id) return res.status(400).json({error: 'Token does not match page'});
    }
    const updates = {};
...
router.delete('/pages/:id', async (req, res) => {
  try {
    ...
    broadcastSSE('page_switched', { activePage: (await storage.getActivePage()), pages: remaining });
    res.json({
      success: true,
      pages: remaining,
      activePage: (await storage.getActivePage())
